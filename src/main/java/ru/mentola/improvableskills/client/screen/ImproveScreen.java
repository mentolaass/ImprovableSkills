...
import ru.mentola.improvableskills.client.screen.element.SkillElement;
...
public final class ImproveScreen extends Screen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
                ...
                if (Util.pointedTo(scaledWidth - 160, scaledHeight - 50, 130, 20, mouseX, mouseY)
                    ...
                    if (!playerData.containsSkill(selectedSkill.getSkill().getId()))
                        if (playerData.getPoints() >= selectedSkill.getSkill().getPricePoints())
                            if (playerData.getLevel() >= selectedSkill.getSkill().getNeedLevel()) {
        ...
        if (playerData != null) {
            ...
            if (Util.pointedTo(25 + maxWidth, 10, 32, 32, mouseX, mouseY) && !upAnimation) {
            ...
            for (int y = 100; y < MinecraftClient.getInstance().getWindow().getScaledHeight() - 100; y+=96) {
                for (int x = 100; x < MinecraftClient.getInstance().getWindow().getScaledWidth() - 100; x+=96) {
                    if (Util.pointedTo(startX + x, startY + y, 32, 32, mouseX, mouseY)) {
                        selectedSkill = skillElements.get(position);
                        return super.mouseClicked(mouseX, mouseY, button);
                    }
                    position++;
    ...
    private void renderInfo(DrawContext context, double mouseX, double mouseY, double scaledHeight, double scaledWidth) {
        ...
        if (playerData != null) {
            ...
            int height = 200 + (this.selectedSkill != null ? (playerData.containsSkill(this.selectedSkill.getSkill().getId()) ? this.selectedSkill.getSkill().getAttributes().size() * 10 : 0) : 0);
            RenderUtil.renderRectangle(context, scaledWidth - 170, scaledHeight - 20 - height, 150, height, new Color(40, 40, 40, 200));
            RenderUtil.renderRectangle(context, scaledWidth - 170, scaledHeight - 20 - height, 150, 20, new Color(30, 30, 30, 200));
            Text title = selectedSkill == null ? Constants.INFO_SKILL : selectedSkill.getSkill().getName();
            context.drawText(MinecraftClient.getInstance().textRenderer, title, (int) (scaledWidth - 170 + (double) 150 / 2) - textRenderer.getWidth(title) / 2, (int) (scaledHeight - height - 13), new Color(190, 190, 190).getRGB(), false);
            RenderUtil.renderBorder(context, scaledWidth - 170, scaledHeight - 20 - height, 150, height, 1, new Color(50, 50, 50));
            if (selectedSkill != null) {
                ...
                for (OrderedText orderedText : wrappedText) {
                    context.drawText(MinecraftClient.getInstance().textRenderer, orderedText, (int) (scaledWidth - 160), (int) (scaledHeight - height + textY), new Color(190, 190, 190).getRGB(), false);
                    textY += MinecraftClient.getInstance().textRenderer.fontHeight;
                }
                if (playerData.containsSkill(selectedSkill.getSkill().getId())) {
                    textY += 10;
                    Skill skill = playerData.getSkill(selectedSkill.getSkill().getId());
                    context.drawText(MinecraftClient.getInstance().textRenderer, Constants.ATTRIBUTES, (int) (scaledWidth - 160), (int) (scaledHeight - height + textY), new Color(250, 179, 43).getRGB(), false);
                    for (Attribute<?> attribute : skill.getAttributes()) {
                        textY += textRenderer.fontHeight + 2;
                        context.drawText(MinecraftClient.getInstance().textRenderer, Text.of(attribute.getName().getString() + " " + attribute.getLevel()), (int) (scaledWidth - 160), (int) (scaledHeight - height + textY), new Color(190, 190, 190).getRGB(), false);
                    };
                }
